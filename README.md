...
* The cumulative regret is $R_T = \Sigma^T_{t=1} |f^*(x_t) - f(x_t)|$, this should be bounded for the algorithm to converge.
...
* use `MSELoss`, no L2-reg since we don't use weight decay: $m\lambda \|\theta - \theta^{(0)}\|_2^2$
